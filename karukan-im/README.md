...
| Space（何も入力していないとき） | 半角スペースを入力（既定）。全角は Ctrl+Space。`bare_space_halfwidth = false` にすると全角スペースになる |
...
bare_space_halfwidth = true    # 何も入力していないときの Space を半角にする（既定ON。false で全角）
